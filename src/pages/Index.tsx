...
const Index = () => {
  ...
  if (accessProtectionEnabled && !isAuthenticated) {
    return <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 relative overflow-hidden">
        {/* Lock toggle button */}
        <Button onClick={toggleAccessProtection} className="absolute top-6 right-6 z-50 bg-gray-800/80 hover:bg-gray-700/80 backdrop-blur-sm border border-gray-600/50 rounded-xl p-3" size="sm">
          <Lock className="w-5 h-5" />
        </Button>

        {/* Animated background particles */}
        <div className="absolute inset-0 opacity-20">
          {[...Array(50)].map((_, i) => <div key={i} className="absolute w-1 h-1 bg-purple-400 rounded-full" style={{
            ...
            <form onSubmit={handleAccessKeySubmit} className="space-y-6">
              <div className="relative group">
                ...
                <div className="relative">
                  ...
                  <Input type="password" placeholder="Enter access key" value={accessKey} onChange={e => setAccessKey(e.target.value)} className="pl-12 bg-black/40 border-gray-600 text-white placeholder-gray-400 h-14 rounded-xl focus:border-purple-500 focus:ring-purple-500 backdrop-blur-sm" required />
                </div>
              </div>
              
              {authError && <div className="text-red-400 text-sm text-center bg-red-500/10 border border-red-500/30 rounded-xl p-4 backdrop-blur-sm animate-shake">
                  {authError}
                </div>}

              <Button type="submit" className="w-full h-14 rounded-xl font-bold text-lg relative overflow-hidden group transition-all duration-300 shadow-2xl">
  ...
  if (isLoading && loadingTool) {
    return <div className="fixed inset-0 bg-black z-50 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-10">
          ...
          {[...Array(20)].map((_, i) => <div key={i} className="absolute border border-purple-500/20 rounded-full" style={{
          width: `${50 + i * 20}px`,
          height: `${50 + i * 20}px`,
          left: '50%',
          top: '50%',
        ...
        <div className="text-center relative z-10">
          ...
          <div className="relative mb-12 group">
            <div className="w-40 h-40 border-8 border-purple-600/20 rounded-full relative">
              ...
              <div className="absolute inset-4 border-8 border-transparent border-t-cyan-400 rounded-full animate-spin" style={{
              animationDirection: 'reverse',
              animationDuration: '1.5s'
            }}></div>
              <div className="absolute inset-8 border-8 border-transparent border-t-purple-300 rounded-full animate-spin" style={{
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-center space-x-3">
              {[...Array(5)].map((_, i) => <div key={i} className="w-3 h-3 bg-purple-500 rounded-full animate-bounce shadow-lg" style={{
    ...
    return <div className="fixed inset-0 bg-black z-50 overflow-hidden">
        ...
        <div className="h-16 bg-black/90 backdrop-blur-xl border-b border-gray-700/50 flex items-center justify-between px-6 relative">
          ...
          <div className="flex items-center space-x-4 relative z-10">
            <Button variant="ghost" size="sm" onClick={closeTool} className="text-gray-400 hover:text-white p-2 hover:bg-white/10 rounded-xl transition-all duration-300 group">
        ...
        <div className="h-[calc(100vh-4rem)] overflow-hidden relative">
          <iframe src={activeTool.url} className="w-full h-full border-0 opacity-0" style={{
          animation: 'fadeIn 0.8s ease-out 0.3s forwards'
        }} title={activeTool.name} sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-top-navigation" />
        </div>
      </div>;
  }

  // Enhanced dashboard with chrome effects and motion graphics
  return <div className="min-h-screen bg-black text-white custom-scrollbar relative overflow-hidden">
      {/* Animated background elements */}
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute inset-0 bg-gradient-to-br from-purple-900/10 via-transparent to-cyan-900/10"></div>
        {[...Array(30)].map((_, i) => <div key={i} className="absolute w-px h-px bg-purple-400/30 rounded-full" style={{
        left: `${Math.random() * 100}%`,
        top: `${Math.random() * 100}%`,
      ...
      <div className="container mx-auto px-6 py-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          return <Card key={tool.id} className="bg-black/40 backdrop-blur-xl border border-gray-700/50 hover:border-purple-500/50 transition-all duration-700 transform hover:scale-105 hover:shadow-2xl cursor-pointer group relative overflow-hidden" onClick={() => !isCurrentlyLoading && openTool(tool.id)}>
                ...
                {/* Animated border */}
                <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-purple-600 via-cyan-600 to-purple-600 opacity-0 group-hover:opacity-30 blur-sm transition-all duration-700" style={{
              animation: 'float 4s ease-in-out infinite'
                ...
                <div className="p-8 relative z-10">
                  ...
                  <div className="flex items-center space-x-4 mb-6">
                    <div className="w-20 h-20 relative group/icon">
                      ...
                      <div className={`absolute inset-2 bg-gradient-to-br ${tool.gradient} rounded-xl flex items-center justify-center shadow-inner`}>
                        {isCurrentlyLoading ? <Loader className="w-8 h-8 text-white animate-spin" /> : <Icon className="w-8 h-8 text-white drop-shadow-lg" />}
                      </div>
                      {/* Chrome reflection */}
                      <div className="absolute inset-2 bg-gradient-to-tr from-white/30 via-white/10 to-transparent rounded-xl pointer-events-none"></div>
                    </div>
                  ...
                  <div className="flex justify-center">
                    <Button className="w-full h-12 rounded-xl font-bold relative overflow-hidden group/btn transition-all duration-300 shadow-xl" onClick={e => {
                  e.stopPropagation();
        ...
        <div className="container mx-auto px-6 py-8 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="flex items-center space-x-6 text-sm text-gray-500">
              ...
              <div className="w-1 h-1 bg-purple-500 rounded-full" style={{
              animation: 'float 2s ease-in-out infinite'
            }}></div>
              <span className="hover:text-cyan-400 transition-colors cursor-pointer">Alpha Mind</span>
              <div className="w-1 h-1 bg-cyan-500 rounded-full" style={{
